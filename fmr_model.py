...
class SolveRegistration(torch.nn.Module):
    ...
    def estimate_t( self, p0, p1, maxiter=5, xtol=1.0e-7, p0_zero_mean=True, p1_zero_mean=True):
        ...
        """
        # for each point (B points altogether), generate transformation-matrix (4 x 4)
        # B 是数据集中物体的总数，N是每个物体的点个数。因此是B x N x 3
        ...
        if p1_zero_mean:
            ...
            q1 = p1
        ## q0 和 q1 是normalized的结果
        # use IC algorithm to estimate the transformation
        g0 = torch.eye(4).to(q0).view(1, 4, 4).expand(q0.size(0), 4, 4).contiguous()#对输入的每一个点云生成RT矩阵存放的位置
    ...
    def ic_algo(self, g0, p0, p1, maxiter, xtol, is_test=False):
        ...
        dt = self.dt.to(p0).expand(batch_size, 6)  # convert to the type of p0. [B, 6] #预备delta t，也就是变换量
        J = self.approx_Jac(p0, f0, dt)
        # compute pinv(J) to solve J*x = -r
        try:
            Jt = J.transpose(1, 2)  # [B, 6, K] J^T
            H = Jt.bmm(J)  # [B, 6, 6] J^T \cdot J
            # H = H + u_lamda * iDentity
            B = self.inverse(H) # (J^T \cdot J) -1
            pinv = B.bmm(Jt)  # [B, 6, K]
        ...
        return r, g, loss_enco_deco # r是F相减的error，g是生成的姿态，loss是branch1用到的chamfer loss结果
    ...
    def approx_Jac(self, p0, f0, dt):
        ...
        for b in range(batch_size): #把这个batch中的每一个点云的transf都算出来，并且转换成6x4x4的形式
            d = torch.diag(dt[b, :])  # [6, 6]
            D = self.exp(-d)  # [6, 4, 4]
            transf[b, :, :, :] = D[:, :, :]
        transf = transf.unsqueeze(2).contiguous()  # [B, 6, 4, 4] 展开成 [B, 6, 1, 4, 4]
        p = self.transform(transf, p0.unsqueeze(1))  # x [B, 1, N, 3] -> [B, 6, N, 3]

        f0 = f0.unsqueeze(-1)  # [B, K, 1]
        f1 = self.encoder(p.view(-1, num_points, 3)) # 这里的num_points 是以p0（source为主的）B x N x 3 > B x K
